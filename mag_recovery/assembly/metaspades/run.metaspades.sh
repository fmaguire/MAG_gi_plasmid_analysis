...
for readset in ../../subsample/perc66.fq ../../subsample/perc100.fq 
do
    name=$(basename $readset | sed 's/\.fq//')
    mkdir -p $name
    #python SPAdes-3.13.0/bin/spades.py --meta -o $name --12 $readset -t 4 
    python SPAdes-3.13.0/bin/spades.py -o $name --continue
    echo "$name done"
done
