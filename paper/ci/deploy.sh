...
envsubst < paper/webpage/README.md > paper/webpage/README-complete.md
mv paper/webpage/README-complete.md paper/webpage/README.md
...
openssl aes-256-cbc \
  ...
  -in paper/ci/deploy.key.enc \
  -out paper/ci/deploy.key -d
chmod 600 paper/ci/deploy.key
ssh-add paper/ci/deploy.key
fi
)
